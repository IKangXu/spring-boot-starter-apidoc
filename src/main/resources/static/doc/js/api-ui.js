    ...
    util.get('/api/api-docs', function (data) {
        if (data.code === 200) {
            if (!($.trim(data.data) === '')) {
                ...
                // 改变标题
                $('.right>.top>span.small-title').text(_result.info.title);
                $('.right>.top>span.version>strong').text($.trim(_result.info.version) === '' ? 'v1.0.0' : _result.info.version);
...
var callback_page_html = function (data) {
    ...
    var _response = data.response, _response_html = '';
    ...
    var _samplecode = data.samplecode;
    $('.dp-highlighter').remove();
    $('.content-body #samplecode').empty().append(_samplecode['javascript']);
